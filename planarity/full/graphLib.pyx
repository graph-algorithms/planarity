...
WRITE_GRAPHML = cgraphLib.WRITE_GRAPHML
...
def gp_GetRandomNumber(int NMin, int NMax) -> int:
    """Generates a pseudo-random integer number in the range of the parameters.
    
    Like ``random.randint()`` except this method calls the same C-layer
    ``gp_GetRandomNumber()`` method that is called by the random graph
    generator methods such as ``gp_CreateRandomGraph()``.

    Args:
        NMin: The lower bound
        NMax: The upper bound

    Returns:
        A pseudo-random integer in the closed range [NMin, NMax]
    """
    return cgraphLib.gp_GetRandomNumber(NMin, NMax)

def seed_rng() -> None:
    # Called by __init__.py to seed the pseudo-random number generator
    # used by gp_GetRandomNumber() and the random graph generator methods.
    # We seed with a random int generated by Python because Python seeds
    # its random number generator with a true source of entropy when 
    # available and only falls back to system time otherwise.
    import random
    srand(random.randint(0, 4294967295))
